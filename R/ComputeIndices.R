...
ComputeIndices = function(Data, Model, FileName, maxDims=6, Folder=NA, Weights="StrataAreas", StrataTable, PlotStrataYearMcmc=TRUE){
  ...
  for(YearI in 1:nrow(PosMean)){
    ...
    Temp = rowSums(t(Area[YearI, ] * t(cMx(Chains[, YearI, ,1]) * cMx(Chains[, YearI, , 2]))))  # In R, straight multiplication ("*") of a vector times a matrix is applied by row, hence the double transpose.
